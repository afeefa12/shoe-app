...
//   >
//     {isOpen ? <FaTimes className="w-6 h-6 text-yellow-700" /> : <FaBars className="w-6 h-6 text-yellow-700" />}
//   </button>
// );

// const AdminSidebar = ({ user, logout }) => {
...
//       {/* Sidebar */}
//       <div
//         className={`fixed top-0 left-0 w-72 h-screen bg-yellow-500 text-gray-800 p-6 flex flex-col shadow-xl transition-transform duration-300 ease-in-out z-40
//           ${isOpen ? 'translate-x-0' : '-translate-x-full'}
//           lg:translate-x-0 lg:w-72`}
//       >
//         {/* Header */}
//         <div className="flex-shrink-0">
//           <h2 className="text-2xl font-extrabold mb-8 pb-4 border-b border-yellow-400 tracking-wide text-center text-gray-900">
//             Admin Panel
//           </h2>
//         </div>
...
//             className="w-full py-3 px-4 bg-yellow-500 hover:bg-yellow-600 rounded-lg transition-all duration-200 ease-in-out font-semibold text-white shadow-md"
//           >
//             Logout
//           </button>
//         </div>
...
const AdminSidebar = ({ user, logout }) => {
  ...
  useEffect(() => {
    ...
    if (!user) {
      ...
      navigate("/adminlogin"); // Redirect to login if no user
    }
  }, [user, location.pathname, navigate]);

  const handleLogout = () => {
    console.log("Logout triggered");
    logout(); // Call the logout function from App.jsx
  ...
  const menuItems = [
    { to: "/admin/dashboard", icon: <FaTachometerAlt />, label: "Dashboard" },
    { to: "/admin/products", icon: <FaBox />, label: "Products" },
    { to: "/admin/orders", icon: <FaShoppingCart />, label: "Orders" }, // Fixed: Removed extra comma
    { to: "/admin/users", icon: <FaUsers />, label: "Users" },
  ];

  if (!user || user.role !== "admin") {
    console.log("Sidebar not rendered - User:", user);
    return null; // Don’t render sidebar if not admin
  }
              ...
              className={`flex items-center gap-4 p-3 rounded-lg transition-all duration-200 ease-in-out
                ${
                  location.pathname === to
                    ...
                    : "text-gray-700 hover:bg-orange-400/80 hover:text-gray-900"
                }`}
              onClick={() => {
                if (window.innerWidth < 1024) {
                  setIsOpen(false);
