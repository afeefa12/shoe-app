...
//                 // Filter for men's products if your API doesn't already do this
...
const Men = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                setLoading(true);
                const response = await axios.get("http://localhost:4000/Products", {
                    params: { categories: "men" } // Query parameter to filter by category
                });
                // If server doesn't support query params, filter client-side
                const menProducts = response.data.filter(product => 
    ...
    const addToCart = async (product) => {
        ...
        try {
            // Optionally verify product exists on server before adding
